...
class TSPTester:
    ...
    def run(self, xe_choice=None):
        ...
        if xe_choice is None:
            xe_choice = self.tester_params['xe_choice']
        else:
            xe_choice = xe_choice   
        ...
        episode = 0
        loop_cnt = 0
        while episode < test_num_episode:  # for batch_num in range(batches_per_epoch):
            ...
            if loop_cnt <= 5:
                self.logger.info(
                    "episode {:3d}/{:3d}, Elapsed[{}], Remain[{}], opt_l:{:.3f}, score:{:.3f}, aug_score:{:.3f}, gap:{:4f}%, aug_gap:{:4f}%, XE_choice:{:1d}".format(
                        episode, test_num_episode, elapsed_time_str, remain_time_str, optimal_reward, score, aug_score,
                        current_gap, aug_current_gap, xe_choice))
            loop_cnt += 1
            ...
            if all_done:
                ...
                self.logger.info(" XE_choice: {:1d} ".format(xe_choice))
    ...
    def _test_one_batch(self, episode, batch_size, xe_choice):
        ...
        return no_aug_score.item(), aug_score.item(), self.optimal_reward.mean().item()
