...
class TSPModel(nn.Module):
    ...
    def pre_forward(self, reset_state, x_edges, x_edges_values, xe_choice):
        ...
        self.encoded_graph = self.graph_encoder(reset_state.problems, x_edges, x_edges_values, xe_choice)  
        self.encoded_nodes = self.node_encoder(reset_state.problems, xe_choice)  
        # shape: (batch, problem, EMBEDDING_DIM)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, input1): 
        Q = self.Wq(input1)  # (batch, pomo, embedding)
        K = self.Wk(input1)  # (batch, pomo, embedding)
        V = self.Wv(input1)  # (batch, pomo, embedding)
        ...
        attention_output = torch.bmm(attention_scores, V)  # (batch, pomo, embedding)
        out1 = self.addAndNormalization1(input1, attention_output)
...
class TSP_GraphEncoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.aggregation = self.model_params['aggregation']  # "mean"

        # Node and edge embedding layers/lookups
        self.nodes_coord_embedding = nn.Linear(2, self.embedding_dim)  
        self.edges_values_embedding = nn.Linear(1, self.embedding_dim // 2)  # , bias=False 
    ...
    def forward(self, data, x_edges, x_edges_values, xe_choice):
        batch_size = data.shape[0]
        problem_size = data.shape[1]

        if xe_choice == 0:   # edge
            x = self.node_encodingblock(x_edges_values)   
        ...
        elif xe_choice == 1:   # node, e=0
            x = self.nodes_coord_embedding(data) 
            e = torch.zeros(batch_size, problem_size, problem_size, self.embedding_dim)
...
class TSP_NodeEncoder(nn.Module):
    ...
    def forward(self, data, xe_choice):  
        # data.shape: (batch, problem, 2)
        batch_size = data.shape[0]
        problem_size = data.shape[1]
        if xe_choice == 0:  # edge
            out = torch.zeros(size=(batch_size, problem_size, self.embedding_dim))  

        elif xe_choice == 1 or xe_choice == 2:  # node / edge+node, e=0
            embedded_input = self.nodes_coord_embedding(data) 
            out = embedded_input
            for layer in self.layers:
                out = layer(out)
        else:
            raise NotImplementedError("Unknown search method")
        return out  # shape: (batch, problem, embedding)
...
class TSP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, encoded_last_graph, ninf_mask):
        ...
        mh_atten_out = mh_atten_out1 + mh_atten_out2  # WA1+WA2
        mh_atten_out = mh_atten_out + q0  
        mh_atten_out = mh_atten_out + self.feedForward(mh_atten_out)  

        single_head_key = self.single_head_key + self.single_head_key_e  # kx+ke
        # shape: (batch, pomo, embedding)
